...
// initial form created  with Validation
  ...
  onSubmits() {
    ...
    if (!this.brandForm.valid) {
      ...
      if (this.imageCaptureUrl != null && this.imageCaptureUrl !== undefined) {
        if (this.imageCaptureUrl.replace(/^.*\./, '') === 'jpeg') {
          this.params.image = '';
   ...
   */
   editManufacturerForm() {
    ...
    if (this.manufacturerEditedValue != null && this.manufacturerEditedValue !== undefined) {
      ...
      this.imageCaptureUrl = this.imageUrl + '?width=160&height=150&path=' + `${this.manufacturerEditedValue.imagePath}` + '&name=' + `${this.manufacturerEditedValue.image}`;
      if (this.manufacturerEditedValue.image || this.manufacturerEditedValue.image === '') {
        this.brandForm.controls['image'].clearValidators();
        this.brandForm.controls['image'].updateValueAndValidity();
      }
