...
export class ZoneAddComponent implements OnInit {
    ...
    private pagenationcount: number = 1;
    ...
    constructor(public fb: FormBuilder, private route: ActivatedRoute, public sandbox: ZoneSandbox, public countrysandbox: CountrySandbox, private router: Router, public service: ZoneService) {
    }
    ...
    onSubmit() {
        ...
        if ((this.zoneForm.value.zonename != '') && (this.zoneForm.value.zonecode != '')) {
            let para: any = {};
    ...
    editZoneList() {
        ...
        if (this.editzoneinfo[0] !== null) {
            if (this.editzoneinfo[0] && this.editzoneinfo[0].name) {
                this.updatetitle = 1;
                console.log('edit info', this.editzoneinfo[0])
