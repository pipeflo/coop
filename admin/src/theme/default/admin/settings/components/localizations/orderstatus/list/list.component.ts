...
export class OrderStatusListComponent implements OnInit {
    ...
    getorderstatuslist(offset: number = 0, keyword) {
        ...
        if (this.pagenationcount) {
            console.log('this.pagecount', this.pagenationcount);
            params.count = true;
    ...
    editOrderStatusList(orderstatusinfo) {
        ...
        this.router.navigate(['/settings/order-status/edit', this.orderstatusdetails.orderStatusId]);
    ...
    addorderstatus() {
        ...
        this.router.navigate(['/settings/order-status/add']);
