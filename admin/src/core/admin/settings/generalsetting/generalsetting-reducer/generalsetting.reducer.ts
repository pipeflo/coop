...
import {GeneralsettingState, GeneralsettingRecordState} from './generalsetting.state';
...
export function reducer(state = initialState, {type, payload}: any): GeneralsettingState {
    if (!type) {
        return state;
    }
...
export const getNewGeneralsettings = (state: GeneralsettingState) => state.newgeneralsettings;
export const getGeneralsettings = (state: GeneralsettingState) => state.getgeneralsetting;
