...
export class StockEffects {
    ...
    // stock list
    @Effect()
    doStockList$: Observable<Action> = this.action$
        .ofType(actions.ActionTypes.DO_STOCKLIST)
        .pipe(
            map((action: actions.DoStockListAction) => action.payload),
            switchMap((state) => {
                return this.stockService.stockList(state)
                    .pipe(
                        switchMap((user) => {
                            return [
                                new actions.DoStockListSuccess((user))
                            ];
                        }),
    ...
    // stock list Count
    @Effect()
    doStockListCount$: Observable<Action> = this.action$
        .ofType(actions.ActionTypes.Do_STOCkLISTCOUNT)
        .pipe(
            map((action: actions.DoStockCount) => action.payload),
            switchMap((state) => {
                return this.stockService.stockListCount(state)
                    .pipe(
                        switchMap((user) => {
                            return [
                                new actions.DoStockCountSuccess(user)
                            ];
                        }),
    ...
    // stock update
    @Effect()
    doStockUpdate$: Observable<Action> = this.action$
        .ofType(actions.ActionTypes.DO_STOCKUPDATE)
        .pipe(
            map((action: actions.DoStockUpdate) => action.payload),
            switchMap((state) => {
                return this.stockService.stockUpdate(state)
                    .pipe(
                        switchMap((user) => {
                            return [
                                new actions.DoStockUpdateSuccess(user)
                            ];
                        }),
    ...
    @Effect()
    doStockDelte$: Observable<Action> = this.action$
        ...
        .pipe(
            ...
            switchMap((state) => {
                ...
                return this.stockService.stockDelete(state, stockStatusId)
                    .pipe(
                        switchMap((user) => [
                            new actions.DoStockDeleteSuccess(user),
                        ]),
