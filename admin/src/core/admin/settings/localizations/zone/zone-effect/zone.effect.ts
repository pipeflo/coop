...
export class ZoneEffect {
    ...
    // NEW ZONE
    @Effect()
    doAddZone$: Observable<Action> = this.action$
        .ofType(actions.ActionTypes.DO_ADD_ZONE)
        .pipe(
            map((action: actions.DoNewZoneAction) => action.payload),
            switchMap((state) => {
                console.log('state', state);
                return this.service.addZone(state)
                    .pipe(
                        switchMap((zone) => [
                            new actions.DoNewZoneSuccessAction(zone),
                        ]),
    ...
    @Effect()
    dozonepagination$: Observable<Action> = this.action$
        ...
        .pipe(
            ...
            switchMap((state) => {
                ...
                return this.service.zonePagiantion(state)
                    .pipe(
                        switchMap((user) => [
                            new actions.GetZoneCountSuccessAction(user),
                        ]),
    ...
    // ZONE LIST
    @Effect()
    dozonelists$: Observable<Action> = this.action$
        .ofType(actions.ActionTypes.GET_ZONE_LIST)
        .pipe(
            map((action: actions.GetZoneListAction) => action.payload),
            switchMap((state) => {
                console.log('effect state', state);
                return this.service.zoneList(state)
                    .pipe(
                        switchMap((user) => [
                            new actions.GetZoneListSuccessAction(user),
                        ]),
    ...
    // Zone Delete
    @Effect()
    doZoneDelete$: Observable<Action> = this.action$
        ...
        .pipe(
            ...
            switchMap((state) => {
                ...
                console.log('effect state', state.zoneId);
                return this.service.deleteZone(state, zoneId)
                    .pipe(
                        switchMap((user) => [
                            new actions.DoZoneDeleteSuccessAction((user)),
                        ]),
