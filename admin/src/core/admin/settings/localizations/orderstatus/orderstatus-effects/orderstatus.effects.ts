...
export class OrderstatusEffects {
  ...
  constructor(private action$: Actions, private apiCli: OrderstatusApiClientService) {
  }

  @Effect()

  doOrderStatuslists$: Observable<Action> = this.action$
    ...
    .pipe(
      ...
      switchMap((state) => {
        ...
        return this.apiCli.getorderstatuslist(state)
          .pipe(
  ...
  @Effect()
  doorderstatuscount$: Observable<Action> = this.action$
    ...
    .pipe(
      ...
      switchMap((state) => {
        console.log(' order  pagination effect state', state);
        return this.apiCli.getorderstatuslist(state)
          .pipe(
            switchMap((user) => [
              new actions.DopaginationorderstatusSuccessAction((user)),
            ]),
            catchError(error => of(new actions.DopaginationorderstatusFailAction(error)))
          );
      })
    );
...
// UPDATE  order STATUS
  @Effect()
  doupdateorderstatus$:Observable<Action> = this.action$
    ...
    .pipe(
      ...
      switchMap((state) => {
        ...
        const Id = state.id;
        return this.apiCli.updateorderstatus(state, Id)
          .pipe(
            map((analysis) => new actions.DoUpdateOrderstatusSuccessAction(analysis)),
            catchError(error => of(new actions.DoUpdateOrderstatusFailAction(error)))
          );
      })
    );
  @Effect()
  doorderstatusDelete$: Observable<Action> = this.action$
    .ofType(actions.ActionTypes.DO_ORDERSTATUS_DELETE)
    .pipe(
      map((action: actions.DoOrderStatusDeleteAction) => action.payload),
      switchMap((state) => {console.log('state', state.orderStatusId);
        const orderStatusId = state.orderStatusId;

        console.log('effect state', state.orderStatusId);
        return this.apiCli.deleteorderstatus(state , orderStatusId)
          .pipe(
            switchMap((user) => [
              new actions.DoOrderStatusDeleteSuccessAction((user)),
            ]),
            catchError(error => of(new actions.DoOrderStatusDeleteFailAction((error))))
          );
      })
    );
}
